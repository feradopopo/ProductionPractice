...
namespace GavrilevAAP
{
    public partial class Form1 : Form
    {
        ...
        {
            if (countGrid == 0)
                button2.Text = "Количество: " + (dataGridView1.Rows.Count - 1).ToString();
            else
                button2.Text = "Количество: " + countGrid.ToString();


            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                int a = Convert.ToInt32(dataGridView1.Rows[i].Cells[4].Value);
                int b = Convert.ToInt32(dataGridView1.Rows[i].Cells[5].Value);
                if (a < b)
                {
